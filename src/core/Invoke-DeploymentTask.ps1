...
try {
    ...
    if (-not $ScriptPath) {
        ...
        if ($SkipIntegrityValidation) {
            Write-DeploymentLog -Message "validación de integridad SHA-256 omitida explÃ­citamente para: $ResolvedScriptPath" -Level "WARNING"
        } else {
        ...
        if ($SkipSignatureValidation) {
            Write-DeploymentLog -Message "ADVERTENCIA DE SEGURIDAD: validación de firma Authenticode omitida explÃ­citamente (PKI no disponible aÃºn) para: $ResolvedScriptPath" -Level "WARNING"
        } else {
